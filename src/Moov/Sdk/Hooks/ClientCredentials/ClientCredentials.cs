...
namespace Moov.Sdk.Hooks.ClientCredentials
{
    ...
    public class ClientCredentialsHook : ISDKInitHook, IBeforeRequestHook, IAfterErrorHook
    {
        ...
        public IMoovClientHttpClient Client = default!;
        ...
        private Credentials? GetCredentials(HookContext hookCtx)
        {
            ...
            if (hookCtx.OperationID == "uploadAvatar")
            {
                return GetCredentialsUploadAvatar(hookCtx.SecuritySource);
            }

            if (hookCtx.OperationID == "deleteAvatar")
            {
                return GetCredentialsDeleteAvatar(hookCtx.SecuritySource);
            }
        ...
        private Credentials? GetCredentialsUploadAvatar(Func<object> securitySource)
        {
            var security = securitySource() as UploadAvatarSecurity;

            if (security?.OAuth2Auth?.ClientID == null || security?.OAuth2Auth?.ClientSecret == null || security?.OAuth2Auth?.TokenURL == null)
            {
                return null;
            }

            return new Credentials(
                security?.OAuth2Auth?.ClientID!,
                security?.OAuth2Auth?.ClientSecret!,
                security?.OAuth2Auth?.TokenURL!,
                null
            );
        }
        ...
        /// <param name="securitySource">A callback function that provides the security object.</param>
        /// <returns>Credentials if available, null otherwise.</returns>
        private Credentials? GetCredentialsDeleteAvatar(Func<object> securitySource)
        {
            var security = securitySource() as DeleteAvatarSecurity;
        ...
        private async Task<Session> DoTokenRequestAsync(string baseURL, Credentials credentials, List<string> scopes, CancellationToken? cancellationToken = null)
        {
            if( Client == null )
            {
                throw new Exception("MoovClientHttpClient not provided");
            }
